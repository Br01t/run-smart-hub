...
const getBMICategory = (bmi: number) => {
  ...
  return { label: "Obesità", color: "text-destructive" };
};
...
const BMICalculator = () => {
  ...
  return (
    <Layout>
      <CalculatorShell
        title="Calcolatore BMI Avanzato"
        description="Calcola il tuo indice di massa corporea con classificazione dettagliata."
        icon={Scale}
      >
        <div className="rounded-xl border border-border bg-card p-6 shadow-card">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground">Altezza (cm)</label>
              <input
                type="number"
                value={height}
        ...
        {result && (
          <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 p-6">
            ...
            <div className="flex flex-col items-center gap-2 rounded-lg bg-card p-6 shadow-card">
              <p className="text-xs font-medium uppercase text-muted-foreground">Il tuo BMI</p>
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              Il BMI è un indicatore generale e non tiene conto di massa muscolare, composizione corporea e altri fattori.
            </p>
          </div>
        )}
